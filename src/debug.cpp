...
static void logEvents(const Ship& ship) {
    ...
    if (!onGround && wasOnGround) Serial.println("[EV] Взлёт");
    ...
    if (!thrusting && wasThrusting) Serial.println("[EV] Тяга выключена");
...
static void logStatus(const Ship& ship) {
    ...
    Serial.printf("[ST] alt=%.0fм  v=%.1f  вл=%.1f  угол=%.0f  топ=%d%%\n",
                  alt, speed, vLocal,
                  ship.angle * 57.2958f,
                  (int)(ship.fuel * 100));
}
